...
bool sendMessageToWristband(String message, int maxRetries = 5) {
  ...
  Serial.println("Retries: " + String(maxRetries));

  for (int attempt = 1; attempt <= maxRetries; attempt++) {
    Serial.println("TX attempt " + String(attempt) + "/" + String(maxRetries));
    
    LoRa.beginPacket();
    LoRa.print(payload);
    LoRa.endPacket();
    LoRa.receive(); // Return to RX mode immediately after TX
    
    messagesSentToEdge++;
    
    if (attempt < maxRetries) {
      // Random backoff between 2-4 seconds to avoid colliding with edge TX
      int waitMs = 2000 + random(500, 2000);
      Serial.println("  Waiting " + String(waitMs) + "ms before next attempt...");
      delay(waitMs);
    }
  }

  Serial.println("✓ Message sent (" + String(maxRetries) + " transmissions)");
