...
// Wristband with MAX30102 + SSD1306 + ESP-NOW integration (fixed send callback signature)
...
// Edge Node MAC (from mac_addresses.txt)
...
void setup() {
  ...
  delay(200);
  Serial.println("\n\nMAX30102 + OLED Heart Rate Monitor + ESP-NOW");
  ...
  if (!particleSensor.begin(Wire, I2C_SPEED_FAST)) {
    ...
    display.display();
    while(1) { Serial.println("MAX30102 Error - Check wiring"); delay(2000); }
  }
...
void initESPNOW() {
  ...
  if (!esp_now_is_peer_exist(edgeNodeMac)) {
    if (esp_now_add_peer(&peerInfo) != ESP_OK) Serial.println("⚠ Failed to add ESP-NOW peer (edge node)");
    else Serial.println("✓ Edge node ESP-NOW peer added");
  } else Serial.println("✓ Edge node peer already exists");
}
...
// onDataSent - signature aligned with platform expectation
void onDataSent(const wifi_tx_info_t *tx_info, esp_now_send_status_t status) {
